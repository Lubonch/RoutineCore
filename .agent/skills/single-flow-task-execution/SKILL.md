...
description: Use when executing implementation plans, handling multiple independent tasks, or doing structured task-by-task development with review gates in a single-flow environment.
---
...
This environment does not support parallel coding subagents. All work happens in a single execution thread.

**Rules:**

1. **One active task only** — never work on multiple tasks simultaneously.
2. **One execution thread only** — no parallel dispatch.
3. **No parallel coding subagents** — do not assume a generic parallel coding tool exists.
4. **Browser automation** may use `browser_subagent` in isolated steps.
5. **Track progress** in chat. If temporary scratch is needed, keep it outside the repo and delete it before completion.
...
digraph process {
    ...
    subgraph cluster_per_task {
      ...
      "Implement, verify, self-review" [shape=box];
      ...
      "Mark task complete in chat" [shape=box];
    }
    ...
    "Suggest Git follow-up only if requested" [shape=box style=filled fillcolor=lightgreen];
    ...
    "Questions about requirements?" -> "Implement, verify, self-review" [label="no"];
    "Implement, verify, self-review" -> "Run spec compliance review (./spec-reviewer-prompt.md)";
    ...
    "Code quality approved?" -> "Mark task complete in chat" [label="yes"];
    "Mark task complete in chat" -> "More tasks remain?";
    ...
    "Run final code review for entire implementation" -> "Suggest Git follow-up only if requested";
}
```
...
- **Clear goal:** Resolve this slice / implement this feature
...
- Run the narrowest permitted validation to check for contradictions between task slices
...
task_boundary:
  ...
  prompt: |
    ...
    - Do not create tests or Git actions unless the user explicitly requested them

    ## Verification
    - Run: [specific verification command or review action allowed by the user's request]
...
- **What verification ran** — diff checks, builds, lint, tests, or manual review as applicable
- **What remains** — remaining tasks, known issues

Report current status in chat.
...
[Report all tasks in chat as 'not_started']
...
Implementation:
  ...
  - Ran permitted verification and captured output
...
Implementation:
  ...
  - Verification completed with the narrowest permitted command
...
[Mark Task 2 complete in chat]

... [Continue through remaining tasks] ...

[After all tasks complete]
[Run final code review on entire implementation]
Final review: All requirements met, ready for user review

[Offer branch integration options only if the user asks]
Done!
```
...
1. Ensure all task entries have been reported as `done` or `cancelled` in chat
2. Run the best permitted validation for the touched slices
3. Verify no contradictions across all tasks
...
- **finishing-a-development-branch** — Use only if the user explicitly asks for branch integration actions

**Should also use:**

- **test-driven-development** — Use only when the user explicitly asks for TDD or test-first work
